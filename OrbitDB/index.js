...
const main = async () => {
    ...
    const directoryPath = path.join(__dirname, "testdb", 'orbitdb');
    const orbitdb = await createOrbitDB({ ipfs, id: `nodejs-${id}`, directory: `./dbtest/orbitdb/${id}` })
    ...
    if (process.argv[2]) {
        console.log('update', process.argv[2])
